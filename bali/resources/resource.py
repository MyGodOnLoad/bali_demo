...
class Resource(metaclass=ResourceMeta):
    ...
    def get_filters(self, schema_in):
        query = self.model.query()
        for backend in self.filter_backends:
            query = backend().get_filters(self, schema_in, query)
        return query.all()
    ...
    def get_endpoint(
        ...
        def endpoint_schema(
            ...
            if 'get' in methods and schema_in_annotation:
                schema_in = schema_in_annotation()

                query_params = dict(resource._request.query_params)
                for k, v in query_params.items():
                    if hasattr(schema_in, k):
                        setattr(schema_in, k, v)
                    else:
                        schema_in.filters[k] = v
